...
This note turns the release playbook into a simple day-to-day format for commits and pull requests.
...
docs(core): define 1.0.0 as the minimum viable framework identity
test(console): cover SQLite migration down selection
refactor(core): enforce module export boundaries during controller injection
```
...
Define 1.0.0 as AssegaiPHP's minimum viable identity
Add package-driven ORM installer discovery
```
...
### For the Assegai organization later

- create or update the shared `.github` repository
- move the PR template and any other default contribution files there
- keep repo-specific overrides only where they are genuinely needed
