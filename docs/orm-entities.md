...
- `dataSource: 'cinema'` ties the entity to the named data source
...
## SQL-specific storage options

Keep ordinary entity metadata focused on shared persistence concerns such as table name, primary keys, columns, and data source selection.

When you need SQL-only storage behavior like engine selection, schema qualification, or SQLite `WITHOUT ROWID`, attach the SQL companion attribute instead:
...
Older `#[Entity(engine: ...)]`, `#[Entity(schema: ...)]`, and `#[Entity(withRowId: ...)]` declarations still work for compatibility, but new code should keep SQL-only storage knobs on `#[SqlEntityOptions(...)]`.
